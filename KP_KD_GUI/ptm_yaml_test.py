...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print("PTM CONTROL - Parameters from YAML")
    print(f"{'='*70}")
    print(f"Motor ID: {MOTOR_ID}")
    print(f"Target Position: {target_pos} rad")
    print(f"Target Velocity: {target_vel} rad/s")
    print(f"Feed-forward Torque: {target_torque} Nm")
    print(f"Kp: {kp_gain}, Kd: {kd_gain}")
    print(f"Control Duration: {CONTROL_DURATION}s")
    print(f"{'='*70}\n")

    # Plotting Setup - 3 subplots for position, velocity, and torque
    plt.ion()
    ...
    vel_data = deque(maxlen=200)
    target_vel_data = deque(maxlen=200)
    torque_data = deque(maxlen=200)
    target_torque_data = deque(maxlen=200)

    # Position plot
    line_pos, = ax1.plot([], [], 'b-', label='Actual', linewidth=2)
    line_target_pos, = ax1.plot([], [], 'r--', label='Target', linewidth=2)
    ...
    # Velocity plot
    line_vel, = ax2.plot([], [], 'g-', label='Actual', linewidth=2)
    line_target_vel, = ax2.plot([], [], 'orange', linestyle='--', label='Target', linewidth=2)
    ax2.set_ylabel('Velocity (rad/s)')
    ax2.legend(loc='upper right')
    ax2.grid(True, alpha=0.3)

    # Torque plot
    line_torque, = ax3.plot([], [], 'b-', label='Actual', linewidth=2)
    line_target_torque, = ax3.plot([], [], 'm--', label='Target', linewidth=2)
    ...
    # Load USB Config (will fallback to param_port from YAML if usb.json not found)
    try:
    ...
    # Initialize motor with YAML motor_id
    revo = Motor("revo_motor", motor_id=MOTOR_ID, type_name="REVO")
    ...
    # PTM Control parameters loaded from YAML above
    ...
    try:
        while time.time() - start < CONTROL_DURATION:
            ...
            line_vel.set_data(time_data, vel_data)
            line_target_vel.set_data(time_data, target_vel_data)
            ax2.relim()
            ax2.autoscale_view()
