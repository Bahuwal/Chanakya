...
class MotorController:
    ...
    def __send_data(self, motor_id, data):
        """Send CAN message"""
        data_padded = data[:8] if len(data) >= 8 else (data + bytes(8 - len(data)))
        msg = can.Message(arbitration_id=motor_id, data=data_padded, is_extended_id=False)
        try:
...
def load_usb_config(path="usb.json"):
    """Load config for param port only (motor uses CAN)"""
    default = {
...
if __name__ == "__main__":
    ...
    # Load USB Config for param port only
    param_port, BAUDRATE = load_usb_config("usb.json")
    print(f"Using param_port={param_port}, baudrate={BAUDRATE}")
    print(f"Using CAN interface: can0 @ 1Mbps")

    # Create CAN bus for motor commands
    try:
        motor_can = can.Bus(channel='can0', interface='socketcan', bitrate=1000000)
        print(f"✓ CAN interface opened")
    except Exception as e:
        print(f"[main] Failed to create CAN bus: {e}")
        raise SystemExit("CAN bus not available.")
