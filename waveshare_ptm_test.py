"""
Waveshare CAN Adapter - PTM Mode Test

This is a TEST VERSION modified to use the Waveshare USB-CAN adapter protocol.

PROTOCOL CHANGES FROM JASON/MOTOREVO:
- Baudrate: 921,600 bps → 2,000,000 bps (2 Mbps)
- Init command: AA 55 12 01 (configure CAN to 1Mbps)
- Header: AA 01 → AA C8
- Motor ID position: End (4 bytes) → Start (2 bytes, little-endian)
- Packet tail: F4 → 55
- Packet length: 15 bytes → 13 bytes

HARDWARE SETUP:
1. Connect Waveshare adapter to Jetson USB port
2. Adapter should appear at /dev/ttyUSB0
3. Connect CAN_H (yellow) and CAN_L (green) to motor bus
4. Add 120Ω termination resistors at both ends

USAGE:
1. Update usb.json with correct port:
   {
...
3. If successful, you can integrate changes into main can_motor_controller.py
...
class MotorController:
    """Motor Controller using Waveshare CAN Adapter Protocol"""
    ...
    def __init__(self, serial_device):
        ...
        # WAVESHARE PROTOCOL: 13 bytes
        # AA C8 [ID_L ID_H] [8 data bytes] 55
        self.tx_buffer = bytearray([
            0xAA,        # Header
            0xC8,        # Control (Standard Frame + Data Frame + 8 Bytes)
            0x00, 0x00,  # Motor ID (2 bytes, little-endian)
        ...
        try:
            ...
            # WAVESHARE INIT: Configure adapter to 1Mbps CAN baudrate
            config_cmd = bytes([0xAA, 0x55, 0x12, 0x01])
            self.serial_device.write(config_cmd)
            sleep(0.1)  # Wait for configuration
            print("✓ Waveshare adapter configured to 1Mbps CAN")
    ...
    def __send_data(self, motor_id, data):
        """Send data using Waveshare protocol (ID at start, 2 bytes)"""
        # WAVESHARE: Motor ID at bytes [2:4], little-endian
        self.tx_buffer[2] = motor_id & 0xFF          # ID Low
        self.tx_buffer[3] = (motor_id >> 8) & 0xFF   # ID High

        # Data at bytes [4:12]
        self.tx_buffer[4:12] = data[:8] if len(data) >= 8 else (data + bytes(8 - len(data)))
    ...
    def __recv_data(self, type, motor_id):
        """Receive and parse Waveshare protocol frames"""
        try:
        ...
        if len(data_recv) > 0 and not hasattr(self, '_debug_printed'):
            print(f"\n[DEBUG] Waveshare received {len(data_recv)} bytes:")
            print(f"  Hex: {data_recv.hex(' ')}")
            self._debug_printed = True

        packets = self.__extract_packets_waveshare(data_recv)

        # Handle remainder bytes (Waveshare frames are 13 bytes)
        frame_length = 13
    ...
    def __extract_packets_waveshare(self, data):
        """Extract Waveshare frames: AA C8 [ID_L ID_H] [8 data] 55"""
        frames = []  # Returns list of (motor_id, data_bytes) tuples
        frame_length = 13
        i = 0

        while i <= len(data) - frame_length:
            # Look for Waveshare header (AA C8)
            if data[i] == 0xAA and data[i+1] == 0xC8:
                # Check for tail (55) at correct position
                if data[i+12] == 0x55:
                    # Extract motor ID (little-endian, 16-bit)
                    motor_id = data[i+2] | (data[i+3] << 8)
                    # Extract 8 data bytes
                    motor_data = data[i+4:i+12]
                    frames.append((motor_id, motor_data))
                    i += frame_length
                else:
                    i += 1  # Invalid frame, search for next header
            else:
                i += 1
